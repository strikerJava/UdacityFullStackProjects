...
def getPopularArticles():
    ...
    print("------------------------")
    print('Article 1: ' + str(result[0][0]) + ' with '
          + str(result[0][1]) + ' Hits!')
    print('Article 2: ' + str(result[1][0]) + ' with '
          + str(result[1][1]) + ' Hits!')
    print('Article 3: ' + str(result[2][0]) + ' with '
          + str(result[2][1]) + ' Hits!')
    print("------------------------")
    db.close()
    print(" ")
...
def getPopularAuthors():
    ...
    db = psycopg2.connect(database=DBNEWS)
    c = db.cursor()
    c.execute("SELECT authors.name, COUNT(author) FROM authors, articles, log"
              + " WHERE articles.slug = (SUBSTRING(path, 10 , LENGTH(path)-1))"
    ...
    print("------------------------")
    print('Author 1: ' + str(result[0][0]) + ' with '
          + str(result[0][1]) + ' Hits!')
    print('Author 2: ' + str(result[1][0]) + ' with '
          + str(result[1][1]) + ' Hits!')
    print('Author 3: ' + str(result[2][0]) + ' with '
          + str(result[2][1]) + ' Hits!')
    print('Author 4: ' + str(result[3][0]) + ' with '
          + str(result[3][1]) + ' Hits!')
    print("------------------------")
    print(" ")
...
def getDaysWithErrors():
    ...
    while x < 60:
        ...
        if percent > .01:
            percentReduce = round((percent * 100), 2)
            print(str(result[0][2]) + ': '
                  + str(percentReduce) + ' % error rate.')
        x += 2
    db.close()
    print("------------------------")
    print("")
